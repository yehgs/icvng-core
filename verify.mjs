...
assert.equal(
  isFiveWeekDeliveryCategory("COFFEE", "some-other-category"),
  false,
);
...

{
  ...
  const client = mockApiClient([{ success: true, requiresVerification: true }]);
...
guestCart = addItemToGuestCart(guestCart, {
  productId: "p1",
  quantity: 1,
  priceOption: "3weeks",
});
...
// ── server cart (logged-in) ──────────────────────────────────────────────
{
  ...
  const client = mockApiClient([
    { success: true, data: [{ _id: "c1", productId: "p1", quantity: 2 }] },
  ]);
...
{
  let guestWishlist = [];
  let result = toggleWishlistGuest(guestWishlist, {
    _id: "p1",
    name: "Coffee",
  });
  assert.equal(result.added, true);
...
// difference from wishlist) ───────────────────────────────────────────────
{
  ...
  const status = await checkCompareStatus({
    apiClient: client,
    endpoints,
    productId: "p1",
  });
...
{
  const client = mockApiClient([
    { success: true, data: { _id: "order1", isParentOrder: true } },
  ]);
...
}
{
  ...
  const client = mockApiClient([
    { success: true, paymentUrl: "https://checkout.paystack.com/abc123" },
  ]);
  const result = await submitGatewayOrder({
    apiClient: client,
    endpoints,
...
}
{
  ...
  assert.deepEqual(
    sorted.map((m) => m.code),
    ["standard", "express", "pickup1"],
  );
}
{
  // reconciliation: previous choice still available -> kept
  const methods = [
    { code: "standard", cost: 600 },
    { code: "express", cost: 2000 },
  ];
  const kept = reconcileSelectedShippingMethod(methods, {
    code: "express",
    cost: 1900,
  });
  ...
  const fallback = reconcileSelectedShippingMethod(methods, {
    code: "overnight",
  });
...
}
{
  const client = mockApiClient([
    {
      success: true,
      data: { methods: [{ code: "standard", type: "delivery", cost: 500 }] },
    },
  ]);
...
}
{
  ...
  const client = async () => {
    called = true;
    return { data: { success: true, data: { methods: [] } } };
  };
  const result = await calculateShipping({
    apiClient: client,
    endpoints,
  ...
  const client = mockApiClient([
    {
      success: true,
      data: [
        { _id: "a1", is_primary: false },
        { _id: "a2", is_primary: true },
      ],
    },
  ]);
...
console.log("All @yehgs/icvng-core checks passed.");
